...
GenerateTypings(schema).then(({ typeStore }) => {
  const coupleLineBreaks = new Array(20).join(' *\n');
  const discouragement = `/*${coupleLineBreaks} * DO NOT EDIT THIS FILE DIRECTLY! - GENERATE IT USING npm run gen-api-types INSTEAD\n${coupleLineBreaks} */\n`;
  fs.writeFileSync('src/app/types/api.ts', discouragement + '\n' + typeStore.toString());
});
