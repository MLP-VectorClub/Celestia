...
const SearchBar: VFC<PropTypes> = ({ initialQuery, guide }) => {
  ...
  const handleInputFocusChange = useCallback((isFocused: boolean) => (e: FocusEvent<HTMLElement>) => {
  ...
  const handleResultKeyDown = (index: number): KeyboardEventHandler<HTMLElement> => e => {
    if (searchInputDirty && e.key === 'Escape') {
      e.preventDefault();
      setActiveResult(null);
      setSearchInputDirty(false);
                ...
                acResultsExist && (
                  <>
                    <ListGroupItem className="text-center p-2" disabled role="option" aria-disabled>
                      Highlight suggestions with <kbd><InlineIcon icon="arrow-up" size="sm" /></kbd>
                      {' '}
                      <kbd><InlineIcon icon="arrow-down" size="sm" /></kbd>, then press <kbd>Enter</kbd> to accept.
                      Pressing <kbd>Enter</kbd> without highlighting one will take you to the search results page.
                      This box can be closed with <kbd>Esc</kbd>
                    </ListGroupItem>
                    {results!.map((r, i) => (
